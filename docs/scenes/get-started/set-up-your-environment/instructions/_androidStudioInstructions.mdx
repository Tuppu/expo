...
Finally, make sure that you can run `adb` from the PowerShell after reboot Windows. For example, run the `adb --version` to see which version of the `adb` your system is running.
