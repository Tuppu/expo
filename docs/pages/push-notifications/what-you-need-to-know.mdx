...
Whether you are starting with notifications or have existing knowledge, this document aims to give you a quick start into the topic by explaining the different kinds of notifications and notification behaviors you should understand.

Remember that Expo's support for notifications builds on top of native functionality for Android and iOS. That means the concepts and behaviors that apply to a regular native app also apply to an Expo app. If unsure about how a specific notification feature works on Android or iOS, see the [official documentation](#external-references) for the respective platform.
...
| ---------- | ----------------------------------------------------------------------- |
| Foreground | `NotificationReceivedListener` & `NotificationResponseReceivedListener` |
...
Headless Background Notifications have the ability to run custom JavaScript in response to a notification _even when app is terminated_. This is powerful but comes with a limitation: even when the notification is delivered to the device, the OS does not guarantee its delivery to your app. This may happen due to a variety of reasons such as when [Doze mode](https://developer.android.com/training/monitoring-device-state/doze-standby) is enabled on Android, or when you send too many background notifications - Apple recommends to not ["send more than two or three per hour"](https://developer.apple.com/documentation/usernotifications/pushing-background-updates-to-your-app#overview).
